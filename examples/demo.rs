...
use aicent::{
    BrainError, CognitiveOrchestrator, CognitivePulse, ImmunityReflex, SomaticActuation, AID,
    VERSION,
};
...
impl ImmunityReflex for SovereignImmunity {
    fn verify_integrity(&self, pulse: &CognitivePulse) -> bool {
        ...
        println!(
            "   ↳ [IMMUNE-SCAN]: Verified Watermark for AID {} ✅",
            pulse.aid
        );
        true
    }
...
impl SomaticActuation for SovereignBody {
    fn request_torque(&self, pulse: &CognitivePulse) -> Result<(), String> {
        ...
        println!(
            "   ↳ [BODY-TORQUE]: Collapsing Intent 0x{:02x?} to physical action...",
            &pulse.intent_hash[..4]
        );
        Ok(())
    }
...
fn main() {
    ...
    let result =
        radiant_brain.execute_reflex_arc("Execute high-torque kinetic alignment", &immunity, &body);

    match result {
        Ok(sharding_latency) => {
            println!(
                "   ↳ Sharding Finality:  {:?} (Internal Logic)",
                sharding_latency
            );
            println!(
                "   ↳ Total Arc Latency:  {:?} (E2E Reflex)",
                start_radiant.elapsed()
            );
    ...
    let _ = ghost_brain.execute_reflex_arc("Standard Operation", &immunity, &body);

    println!(
        "   ↳ Total Arc Latency:  {:?} (Legacy Emulation Enforced)",
        start_ghost.elapsed()
    );
